...
#include "ISettingStrategyDetailCustomizer.h"

class UConsoleVariableSettingApplicationStrategy;

/**
 * Detail customizer for ConsoleVariableSettingApplicationStrategy that provides
 * a console variable name field and related info panels
 */
class FConsoleVariableDetailCustomizer : public ISettingStrategyDetailCustomizer
{
public:
	virtual bool CanHandle(const USettingApplicationStrategy* Strategy) const override;
...
private:
	...
	FString GetConsoleVariableName(
		const UConsoleVariableSettingApplicationStrategy* CVarStrategy,
		TSharedRef<IPropertyHandle> SubKeyHandle,
