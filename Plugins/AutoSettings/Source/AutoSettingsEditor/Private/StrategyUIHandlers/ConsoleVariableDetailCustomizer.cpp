...
bool FConsoleVariableDetailCustomizer::CanHandle(const USettingApplicationStrategy* Strategy) const
{
	return Strategy && Strategy->IsA<UConsoleVariableSettingApplicationStrategy>();
}
...
{
	const auto* CVarStrategy = Cast<UConsoleVariableSettingApplicationStrategy>(Strategy);
	if (!CVarStrategy)
	{
		return;
	}
