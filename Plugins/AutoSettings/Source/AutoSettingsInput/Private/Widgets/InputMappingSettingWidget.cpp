...
void UInputMappingSettingWidget::UpdateSettingKey()
{
	...
	SetSetting(SettingType, NewKeyExtension);
}
...
void UInputMappingSettingWidget::StartCapture()
{
	...
	InputProcessor->OnKeySelected.AddWeakLambda(this, [this](FCapturedInput CapturedInput)
	{
		...
		for (auto GlobalContext : GetDefault<UAutoSettingsInputConfig>()->GlobalMappingContexts)
		{
		...
		for (auto ExistingMapping : ExistingMappingsWithKey)
		{
		...
		if (MappingAttemptInfo.Issues.Num() > 0)
		{
			...
			DetermineResolutionAsync(MappingAttemptInfo).Then([WeakThis, CapturedInput, MappingAttemptInfo](auto Future)
			{
				...
				if (bRemoveExistingMapping)
				{
					...
					for (auto Issue : MappingAttemptInfo.Issues)
					{
...
{
	// Pass the captured input information to the application strategy
	// Format: "KeyName:AxisScale" so the application strategy can determine directional intent
