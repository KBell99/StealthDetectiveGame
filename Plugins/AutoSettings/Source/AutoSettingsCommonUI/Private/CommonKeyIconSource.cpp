...
FName GetGamepadName(const UCommonInputSubsystem* Subsystem)
{
	...
	for (auto ControllerDataClass : UCommonInputPlatformSettings::Get()->GetControllerData())
	{
...
FSlateBrush UCommonKeyIconSource::GetIconBrushForKey(const FKey& Key, const ULocalPlayer* Player) const
{
	...
	if (Key.IsValid() && UCommonInputPlatformSettings::Get()->TryGetInputBrush(SlateBrush, Key, GetInputType(Key),
		    GetGamepadName(Player)))
	{
		return SlateBrush;
	}
